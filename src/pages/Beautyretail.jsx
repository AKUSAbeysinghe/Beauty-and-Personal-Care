...
// Hardcoded High-Quality Images
const hardcodedImages = {
  hero: BeautyRetail,
  cosmetics: 'https://images.unsplash.com/photo-1591370874773-6702e0a0d9d0?q=80&w=2070',
  sephora: 'https://images.unsplash.com/photo-1625772299848-2b8f2e1e3b3e?q=80&w=2070',
  ulta: 'https://images.unsplash.com/photo-1556228720-195a672ee8b6?q=80&w=2070',
  makeup: 'https://images.unsplash.com/photo-1512499617640-c2f1e8f9d1e3?q=80&w=2070',
  default: 'https://images.unsplash.com/photo-1515378791036-0648a3ef77b2?q=80&w=2070'
};

// Fallback Services with hardcoded images
const fallbackServices = [
  { id: 1, category_id: 2, sub_category_id: 5, name: 'Premium Lipstick', price: 2500, image_url: hardcodedImages.cosmetics },
  { id: 2, category_id: 2, sub_category_id: 6, name: 'Sephora Foundation', price: 4000, image_url: hardcodedImages.sephora },
  { id: 3, category_id: 2, sub_category_id: 7, name: 'Ulta Mascara', price: 3000, image_url: hardcodedImages.ulta },
  { id: 4, category_id: 2, sub_category_id: 8, name: 'Hydrating Serum', price: 5000, image_url: hardcodedImages.makeup },
];
...
const BeautyRetailPage = () => {
  ...
  const fetchServices = async () => {
    ...
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        const retailServices = data.data.filter(s => String(s.category_id) === '2');
        setServices(retailServices.length > 0 ? retailServices : fallbackServices);
      } else {
        setServices(fallbackServices);
      }
    } catch (err) {
  ...
  // Categorize services
  const cosmeticsRetailServices = services.filter(s => String(s.sub_category_id) === '5');
  const sephoraServices = services.filter(s => String(s.sub_category_id) === '6');
  const ultaServices = services.filter(s => String(s.sub_category_id) === '7');
  const makeupSkincareServices = services.filter(s => String(s.sub_category_id) === '8');
  ...
  const getMinPrice = (arr) => {
    if (!arr || arr.length === 0) return 2500;
    const prices = arr.map(s => Number(s.price) || 0).filter(p => p > 0);
    return prices.length > 0 ? Math.min(...prices) : 2500;
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      >
        <div className="absolute inset-0 bg-black bg-opacity-50" />
        <div className="relative z-10 flex flex-col items-center text-center text-white p-6 max-w-4xl mx-auto">
          <h1 className="text-5xl md:text-7xl font-black uppercase tracking-tighter">Cosmetics Retail</h1>
          <p className="text-lg md:text-xl text-gray-200 mt-4 max-w-2xl">
            Shop premium beauty products from top brands like Sephora and Ulta Beauty
          </p>
      ...
      <section className="py-16 sm:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-4xl font-black uppercase text-gray-900">Cosmetics Retail</h2>
            <p className="mt-4 text-lg text-gray-600">Shop a wide range of high-quality beauty products</p>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <img
                src={getSubCategoryImage(cosmeticsRetailServices, 'cosmetics')}
                alt="Cosmetics Retail"
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-4 mb-4">
                <span className="bg-black text-white p-3 rounded-full"><IconSparkles className="h-6 w-6" /></span>
                <h3 className="text-2xl font-bold">General Cosmetics</h3>
              </div>
              {loading ? <p>Loading products...</p> : (
                <ul className="space-y-3 mb-8">
                  {cosmeticsRetailServices.length > 0 ? cosmeticsRetailServices.map(s => (
                    <li key={s.id} className="flex items-center">
                      <IconSparkles className="h-4 w-4 mr-3 text-black" />
                      <span>{s.name}</span>
                    </li>
              ...
              <div className="mt-auto flex items-center justify-between">
                <p className="text-xl font-semibold">From <span className="font-bold">Rs.{getMinPrice(cosmeticsRetailServices)}</span></p>
                <button className="bg-black text-white px-8 py-3 rounded-lg hover:bg-gray-800 transition">Shop Now</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="bg-gray-50 py-16 sm:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-4xl font-black uppercase text-gray-900">Sephora Brands</h2>
            <p className="mt-4 text-lg text-gray-600">Discover Sephora’s exclusive range of makeup and skincare</p>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="flex flex-col">
              <div className="flex items-center gap-4 mb-4">
                <span className="bg-black text-white p-3 rounded-full"><IconSparkles className="h-6 w-6" /></span>
                <h3 className="text-2xl font-bold">Sephora Brands</h3>
              </div>
              {loading ? <p>Loading...</p> : (
                <ul className="space-y-3 mb-8">
                  {sephoraServices.map(s => (
                    <li key={s.id} className="flex items-center">
                      <IconSparkles className="h-4 w-4 mr-3 text-black" /> {s.name}
                    </li>
                  ))}
                </ul>
              )}
              <div className="mt-auto flex justify-between items-center">
                <p className="text-xl font-semibold">From <span className="font-bold">Rs.{getMinPrice(sephoraServices)}</span></p>
                <button className="bg-black text-white px-8 py-3 rounded-lg hover:bg-gray-800 transition">Shop Now</button>
              </div>
            </div>
            <div>
              <img src={getSubCategoryImage(sephoraServices, 'sephora')} alt="Sephora" className="rounded-lg shadow-md w-full h-[420px] object-cover" onError={(e) => e.target.src = hardcodedImages.default} />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 sm:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-4xl font-black uppercase text-gray-900">Ulta Beauty Products</h2>
            <p className="mt-4 text-lg text-gray-600">Explore Ulta Beauty’s premium product lines</p>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <img src={getSubCategoryImage(ultaServices, 'ulta')} alt="Ulta Beauty" className="rounded-lg shadow-md w-full h-[420px] object-cover" onError={(e) => e.target.src = hardcodedImages.default} />
            </div>
            <div className="flex flex-col">
              <div className="flex items-center gap-4 mb-4">
                <span className="bg-black text-white p-3 rounded-full"><IconSparkles className="h-6 w-6" /></span>
                <h3 className="text-2xl font-bold">Ulta Beauty Products</h3>
              </div>
              {loading ? <p>Loading...</p> : (
                <ul className="space-y-3 mb-8">
                  {ultaServices.map(s => (
              ...
              <div className="mt-auto flex justify-between items-center">
                <p className="text-xl font-semibold">From <span className="font-bold">Rs.{getMinPrice(ultaServices)}</span></p>
                <button className="bg-black text-white px-8 py-3 rounded-lg hover:bg-gray-800 transition">Shop Now</button>
              </div>
            </div>
          </div>
      ...
      <section className="bg-gray-50 py-16 sm:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-4xl font-black uppercase text-gray-900">Makeup & Skincare</h2>
            <p className="mt-4 text-lg text-gray-600">High-performance makeup and skincare for radiant skin</p>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="flex flex-col">
              <div className="flex items-center gap-4 mb-4">
                <span className="bg-black text-white p-3 rounded-full"><IconSparkles className="h-6 w-6" /></span>
                <h3 className="text-2xl font-bold">Makeup & Skincare</h3>
              </div>
              {loading ? <p>Loading...</p> : (
                <ul className="space-y-3 mb-8">
                  {makeupSkincareServices.map(s => (
                    <li key={s.id} className="flex items-center">
                      <IconSparkles className="h-4 w-4 mr-3 text-black" /> {s.name}
                    </li>
                  ))}
              ...
              <div className="mt-auto flex justify-between items-center">
                <p className="text-xl font-semibold">From <span className="font-bold">Rs.{getMinPrice(makeupSkincareServices)}</span></p>
                <button className="bg-black text-white px-8 py-3 rounded-lg hover:bg-gray-800 transition">Shop Now</button>
              </div>
            </div>
            <div>
              <img src={getSubCategoryImage(makeupSkincareServices, 'makeup')} alt="Makeup & Skincare" className="rounded-lg shadow-md w-full h-[420px] object-cover" onError={(e) => e.target.src = hardcodedImages.default} />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-gray-50 py-16 sm:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="mt-4 text-lg text-gray-600">Complete your beauty experience</p>
          </div>
      ...
      <section className="bg-black py-16 sm:py-20">
        <div className="max-w-4xl mx-auto text-center px-4">
          <h2 className="text-4xl font-black uppercase text-white">Ready to Elevate Your Beauty Routine?</h2>
          <p className="mt-4 text-lg text-gray-200">Shop premium products today</p>
          <div className="mt-8 flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <a href="#shop" className="px-8 py-3 bg-white text-black rounded-lg hover:bg-gray-200 transition">Shop Now</a>
          </div>
        </div>
      </section>
    </div>
