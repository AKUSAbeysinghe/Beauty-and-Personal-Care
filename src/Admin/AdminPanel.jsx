...
const AdminPanel = () => {
  ...
  // FIXED: Gallery categories with Main & Support
  const galleryCategories = [
    // { value: "all", label: "All Photos" },
    { value: "main", label: "Main Image" },
    { value: "support", label: "Support Images" },
  ...
  const addServiceEntry = () => {
    if (!currentService.name || !currentService.category_id) {
      setError("Warning: Service Name and Category are required.");
      return;
    }
    setFormData((prev) => [
      ...prev,
    ...
    setMessage(`Success: Service "${currentService.name}" added to list!`);
  ...
  const fetchCategories = async () => {
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        ...
        setError("Warning: Using predefined categories.");
      }
    } catch (e) {
      setCategories(predefinedCategories);
      setSubCategories(predefinedSubCategories[predefinedCategories[0].id] || []);
      setError("Warning: Could not fetch categories: " + e.message);
    }
  };
  ...
  const fetchServices = async () => {
    try {
      ...
      if (data.success === false) {
        setError(data.message || "Warning: Could not fetch services. Using predefined services.");
      ...
      setError("Warning: Could not fetch services: " + e.message + ". Using predefined services.");
      setServices(predefinedServices.filter((s) => s.type === "standard"));
    }
  };
  ...
  const fetchGalleryImages = async () => {
    try {
      ...
      setError("Warning: Could not fetch gallery images.");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (isGallery && !imageData.image && !isEditing) {
      setError("Warning: Please select an image.");
    ...
    if (!isGallery) {
      if (formData.length === 0 && !isEditing) {
        setError("Warning: Please add at least one service.");
        setIsSubmitting(false);
        return;
      }
      if (!sharedFields.price || sharedFields.price < 0) {
        setError("Warning: Price is required and must be non-negative.");
        setIsSubmitting(false);
        return;
      }
    }
    ...
    try {
      if (isGallery) {
        ...
        if (!res.ok || !result.success) {
          setError(result.message || "Error: Save failed");
        } else {
          setMessage(isEditing ? "Success: Updated!" : "Success: Saved successfully!");
          setShowForm(false);
        ...
        for (let service of servicesToSubmit) {
          ...
          if (!res.ok || !result.success) {
            setError(result.message || "Error: Failed to save one or more services");
            setIsSubmitting(false);
            return;
          }
          successCount++;
        ...
        if (successCount === servicesToSubmit.length) {
          setMessage(isEditing ? "Success: Updated!" : `Success: ${successCount} service(s) saved successfully!`);
          setShowForm(false);
          setFormData([]);
          await fetchServices();
          setIsEditing(false);
      ...
      setError("Warning: Server error while saving");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.message || "Error: Delete failed");
      }
    } catch (err) {
      setError("Warning: Server error while deleting");
    } finally {
